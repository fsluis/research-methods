...
author: "Frans van der Sluis and Mara Günther"
date: "`r Sys.Date()`"
output: 
  html_document:
    css: styles.css
...
## LAB SESSION 1: DESCRIPTIVE STATISTICS
...
```{r load-readxl}
install.packages("readxl", repo="http://cran.dk.r-project.org")
library(readxl)
```
...
Now, let's take a first look at our data. Place the file “questionnaire_data_2023.xlsx” in the directory you are currently working in:
...
```
```{r load-data}
questionnaire_data <- read_excel("questionnaire_data_R_2024.xlsx")
...
questionnaire_data <- questionnaire_data %>%
  mutate(MessagesCategory = case_when(
    Messages_per_day <= 5 ~ "5 or less",
    Messages_per_day <= 10 ~ "6 to 10",
    TRUE ~ "More than 10"
  ))
```
...
## Keyboard data
Let's now input the keyboard data file. It's called 'keyboard_data_R_2024.csv' and can be imported using the command 'read.csv(file="keyboard_data_R_2024.csv")'. Be sure sure to assign the output to a new variable, which we'll call 'keyboard_data'.
<div class="question">
8. Import keyboard_data_R_2024.csv to a variable named keyboard_data.
</div>
...
keyboard_data <- read.csv(file="keyboard_data_R_2024.csv")
```

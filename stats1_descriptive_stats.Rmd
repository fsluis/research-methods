...
author: "Frans van der Sluis and Mara Günther (2026 version)"
date: "`r Sys.Date()`"
output: 
  html_document:
    css: styles.css
...
keyboard_file <- "keyboard_data_R_2026.csv"
questionnaire_file <- "questionnaire_data_R_2026.xlsx" 
# replace '202x' manually as well below, cause it's in a code block
```
...
First, install R and RStudio following this [link](https://posit.co/download/rstudio-desktop/). 

Once R and RStudio have been installed, the next step is to install and load libraries that we will be using this session. You can read more about libraries [here](https://moderndive.com/v2/getting-started.html). The `install.packages` command installs a package, you only need to run this once (until you re-install or upgrade R). The `library` command needs to run everytime you start a new R session.
